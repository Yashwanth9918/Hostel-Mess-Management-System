...
export const createMenu = async (user, menuData) => {
  const { messId, weekStartDate, weekEndDate, weekNumber, year, dailyMenus, announcement } = menuData;

  // Managers can only create menus for their own mess
  if (user.role === 'manager' && user.messId !== messId) {
    throw new AppError('You can only create menus for your assigned mess', 403);
  }
...
/**
 * Get current week menu for a mess.
 */
export const getCurrentMenu = async (messId) => {
...
export const updateMenu = async (user, menuId, updateData) => {
  ...
  if (user.role === 'manager' && user.messId !== menu.messId) {
    throw new AppError('You can only update menus for your assigned mess', 403);
  }
...
export const publishMenu = async (user, menuId) => {
  ...
  if (user.role === 'manager' && user.messId !== menu.messId) {
    throw new AppError('You can only publish menus for your assigned mess', 403);
  }
...
export const deleteMenu = async (user, menuId) => {
  ...
  if (user.role === 'manager' && user.messId !== menu.messId) {
    throw new AppError('You can only delete menus for your assigned mess', 403);
  }
...
/**
 * Get all menus for a mess (paginated).
 */
export const getAllMenusForMess = async (messId, { page = 1, limit = 10 }) => {
 ...
 */
export const getPreviousMenu = async (messId) => {
  ...
  if (!previousMenu) {
    throw new AppError('No previous menu found for this mess', 404);
  }
